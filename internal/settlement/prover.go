...
// ProverV29 is a mock prover for settlement proofs.
type ProverV29 struct{}

func (ProverV29) Finalize(stateRoot string) error {
    if len(stateRoot) == 0 {
        return fmt.Errorf("empty state root")
    }
    return nil
...
// ProverV33 is a mock prover for settlement proofs.
type ProverV33 struct{}

func (ProverV33) Finalize(stateRoot string) error {
    if len(stateRoot) == 0 {
        return fmt.Errorf("empty state root")
    }
    return nil
