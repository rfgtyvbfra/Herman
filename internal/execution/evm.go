...
// EVMEngine v24 provides a mock execution engine.
type EVMEngineV24 struct{}

func (EVMEngineV24) Init(vm VMType) error {
    if vm != VM_EVM {
        return fmt.Errorf("unsupported VM: %s", vm)
    }
    return nil
...
func (EVMEngineV24) Execute(tx []byte) ([]byte, error) {
    // Mock: echo back with prefix
    out := append([]byte("evm24-"), tx...)
    return out, nil
}
package execution

// EVMEngine v28 provides a mock execution engine.
type EVMEngineV28 struct{}

func (EVMEngineV28) Init(vm VMType) error {
    if vm != VM_EVM {
        return fmt.Errorf("unsupported VM: %s", vm)
    }
    return nil
...
func (EVMEngineV28) Execute(tx []byte) ([]byte, error) {
    // Mock: echo back with prefix
...
// EVMEngine v36 provides a mock execution engine.
type EVMEngineV36 struct{}

func (EVMEngineV36) Init(vm VMType) error {
    if vm != VM_EVM {
        return fmt.Errorf("unsupported VM: %s", vm)
    }
    return nil
...
func (EVMEngineV36) Execute(tx []byte) ([]byte, error) {
    // Mock: echo back with prefix
    out := append([]byte("evm36-"), tx...)
    return out, nil
}
